...
# === LOAD POCKET DATA ===
pocket_path = r"/pocket_descriptors_ESR1_dynamic.csv"
df = pd.read_csv(pocket_path)

# === LOAD CLUSTERS FILE ===
cluster_file = r"/out/clusters_ESR1.txt"
...
df.to_csv("pocket_PCA_ESR1_colored.csv", index=False)

print("Explained variance ratio:", pca.explained_variance_ratio_)
print("Saved pocket_PCA_ESR1_colored.csv")
...
plt.xlabel("PC1")
plt.ylabel("PC2")
plt.title("PCA of ESR1 Pocket Geometry (colored by clusters)")
plt.legend()
plt.grid(True)
plt.show()
...
ax.set_ylabel("PC2")
ax.set_zlabel("PC3")
ax.set_title("3D Pocket Geometry PCA – ESR1 (Cluster-colored)")
ax.legend()
