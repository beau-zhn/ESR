...
def main():
    ...
    for r, fp in enumerate(fingerprints):
        ...
        for c, k in enumerate(K_values):
            ...
            scatter = ax.scatter(emb[:, 0], emb[:, 1], c=list(cvals), s=30, alpha=0.9, linewidths=0)

            # Add legend with family names
            handles = [Line2D([0], [0], marker='o', linestyle='', color=color_map[f], label=f)
                    for f in uniq_fams]
            leg = ax.legend(handles=handles, title="Chemical Family",
            ...
            # choose descriptor column for legend
            ...
            leg = ax.legend(handles=handles, title="Clusters", bbox_to_anchor=(1.02, 1), loc='upper left', fontsize=8)
            ax.add_artist(leg)
            ...
            #     ax.text(cx, cy, str(cl), fontsize=11, fontweight='bold',
            ...
            sc2 = ax_tmp.scatter(emb[:, 0], emb[:, 1], c=labels, s=20, cmap='tab10')
            for cl in uniq:
                pts = emb[labels == cl]
                cx, cy = pts.mean(axis=0)
                ax_tmp.text(cx, cy, str(cl), fontsize=11, fontweight='bold',
                            ha='center', va='center',
                            bbox=dict(boxstyle="round,pad=0.2", fc="white", ec="black", alpha=0.6))
